...
const addVisitor = async (req, res) => {
  ...
  try {
    ...
    // 2. SPEED FIX: Send response IMMEDIATELY. Do not wait for email.
    res.status(201).json(visitor);

    // 3. Send Email in "Background"
    // We do not use 'await' here so the user doesn't have to wait for it.
    const message = `Hello ${name},\n\nYour visitor pass for ${purpose} has been generated.\n\nPlease show your QR code at the reception.`;
    sendEmail(email, "Your Visitor Pass Details", message).catch((err) =>
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ message: "Server Error: Could not create visitor" });
    }
  }
};
